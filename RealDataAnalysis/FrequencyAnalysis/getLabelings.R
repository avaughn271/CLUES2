...
for (groupnumber in unique(Data$Paintings)) {
  
ToWrite = ""

for (i in 1:nrow(Data)) {
  if (Data$Paintings[i] == groupnumber) {
...
writeLines(ToWrite, paste0("Haplotypes", toString(CHR), "_group_",toString(groupnumber),".csv"))
}