...
ggplot(LARGEDATAFRAME, aes(x=True_s, y=Inferred_s)) + geom_violin(fill = "lightskyblue", scale = "count") + 
  ...
  ylim(-0.005, 0.015) + labs(y = "Inferred Value of s", x="Simulated Dominance Coefficients (h)") +  
  ggtitle("") + 
  theme(plot.title = element_text(hjust = 0.5)) + geom_segment(aes(x = 0.6, y = .005, xend = 1.4, yend = .005, linetype=`True s`),  
                                                                color = "red", size=0.5) + 
