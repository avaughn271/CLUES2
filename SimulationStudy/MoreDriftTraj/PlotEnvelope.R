...
findconfidenceinterval = function(postss) {
  ...
  extendd = 0
  while(TRUE) {
    if (sum(actualposts[max(1,(-extendd + maxxindex)):(extendd + maxxindex)]) > 0.95   ) {
      return(c(max(1,maxxindex-extendd), maxxindex + extendd, maxxindex))
    }
    extendd = extendd + 1
...
Total = rbind(Total, data.frame(Midpoints, Lower, Upper, Modess, x))
  ...
  geom_ribbon(aes(ymin = Lower, ymax = Upper, fill = "95% Posterior Interval"), alpha = 0.3) +
    ...
    scale_fill_manual("",values="black")+ labs( x="Generations Ago", y = "Derived Allele Frequency") + ggtitle("N = 1000, m = 2, s = 0.005") +
