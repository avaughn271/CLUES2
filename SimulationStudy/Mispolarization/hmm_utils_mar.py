...
def maruyamaMean (x, Ne, s):
    if (np.abs(s) <= 1e-8):
        return - x / (2*Ne)
    else:
        clipEpsilon = 1e-6
        x = np.minimum(1-clipEpsilon, np.maximum(x, clipEpsilon)) #np.clip (x, clipEpsilon, 1-clipEpsilon)
        return - 0.5 * s * x*(1-x) * myCoth (0.5 * 2*Ne*s * (1-x))
