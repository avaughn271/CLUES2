...
ggplot(ss, aes(x=counts, y=s)) + geom_violin(  fill = "lightskyblue") + 
  ...
  ylim(-boundss, boundss) + labs(  y = "Inferred Value of s", x="") +  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()  ) +  geom_hline(yintercept=0.0, linetype="dashed",  color = "red", size=1) + ggtitle("N = 200000, m = 1") +
