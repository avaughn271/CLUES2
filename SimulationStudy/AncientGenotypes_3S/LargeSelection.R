...
LARGEDATAFRAME[,1] = rev(LARGEDATAFRAME[,1])
...
ggplot(LARGEDATAFRAME, aes(x=True_s, y=Inferred_s)) + geom_violin(fill = "lightskyblue", scale = "count") + 
  ...
  geom_segment(aes(x = 0.6, y = -0.005, xend = 1.4, yend =-0.005, linetype=`True s`), color = "red", size=0.5) +
   geom_segment(aes(x = 1.6, y =  0.0, xend = 2.4, yend = 0.0, linetype=`True s`), color = "red", size=.5) + 
  geom_segment(aes(x = 2.6, y = 0.01, xend = 3.4, yend = 0.01, linetype=`True s`), color = "red", size=.5)  +
