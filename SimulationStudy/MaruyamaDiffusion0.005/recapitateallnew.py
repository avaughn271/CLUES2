...
modernallelefreq = float(sys.argv[5])
...
while True:
    ...
    sweep_model = msprime.SweepGenicSelection(
        ...
        end_frequency=modernallelefreq,
        s=sss,  dt=1e-6)
    ...
    for jjjjjj in numpy.unique(IsDerived):
        ListOfPossibleNodes.append(jjjjjj)
        ListOfCounts.append(IsDerived.count(jjjjjj))
    #print(ListOfPossibleNodes, ListOfCounts)
    ListOfCounts = numpy.absolute(numpy.subtract(ListOfCounts , modernallelefreq * NUMBEROFLEAVESTOSAMPLE))
    if min(ListOfCounts) != 0:
        continue
    #print(ListOfCounts)
    #print((min(ListOfCounts)))
    relevantval = (ListOfPossibleNodes[numpy.where(ListOfCounts == (min(ListOfCounts)))[0][0]])

    IsDerived = numpy.equal(IsDerived, relevantval) + 0
