...
for (i in 1:NumberOfTrials) {
  ...
  loglr = c(loglr, as.numeric(substr(Line1, 8, nchar(Line1))))
}
...
plot(actual,experimental, xlim = c(0,1), ylim = c(0,1), pch = 19, col = "steelblue4",
     main = paste0("s = ", toString(TrueS)), xlab = "Theoretical Distribution Function", ylab = "Empirical Distribution Function")
