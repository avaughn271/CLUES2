...
selectioncoef1 = 0.01
selectioncoef2 = 0
selectioncoef3 = -0.005
selectioncoef = rep(selectioncoef1, 10000)
selectioncoef[200:600] = selectioncoef2
selectioncoef[601:9000] = selectioncoef3

mfreq = as.numeric(args[2])
...
OneSim = function(modernfreq) {
  ...
  nextgeneration = 2
  for (ii in 1:1003 ) {
    mu = currentfreq - selectioncoef[ii] * currentfreq * (1-currentfreq)
    currentfreq = rnorm(1, mean = mu, sd = sqrt(currentfreq * (1 - currentfreq)/(2*N)))
